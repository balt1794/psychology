  ...
  execute: async ({ inputData, suspend, resumeData, mastra, writer }) => {
    ...
    if (!approvedChanges?.length) {
      writer.write({
        ...
        data: { changes: [], status: "loading" } satisfies SuggestionStepData,
      });

      const base64Image = await getBase64FromFileUrl(imageUrl);
      const suggestionAgent = mastra.getAgent("interiorImprovementSuggestionAgent");
      ...
      for await (const chunk of stream.objectStream) {
        writer.write({
          type: "data-suggestions",
          data: { changes: chunk.suggestions, status: "streaming" } satisfies SuggestionStepData,
        });
      }
      ...
      writer.write({
        type: "data-suggestions",
        data: { changes: result.suggestions, status: "completed" } satisfies SuggestionStepData,
      });
    ...
    return { imageUrl, changes: approvedChanges ?? [] };
  },
});
...
const interiorImageImprovementStep = createStep({
  ...
  execute: async ({ inputData, mastra, writer }) => {
    ...
    writer.write({
      ...
      data: { status: "in-progess", url: "" } satisfies ImprovementStepData,
    });
    ...
    writer.write({
      type: "data-improvedInterior",
      data: { status: "completed", url } satisfies ImprovementStepData,
    });

    return { improvedImageUrl: url };
  },
});
...
export const interiorImprovementSuggestionWorkflow = createWorkflow({
  ...
  outputSchema: z.object({ improvedImageUrl: z.string() }),
  description:
    "Suggests interior improvements and applies selected edits to generate a redesigned room image.",
})
